...
class MjpegConnector(threading.Thread):
    ...
    def connect(self):
        try:
            ...
            self.stream = requests.get(self.address, stream=True, verify=False,
            ...
            if position != -1:
                ...
                try:
                    self.stream_queue.put(
                        [image_buffer, min_ts, max_ts, events, amplification, image_length], False, 0)
                except queue.Full:
                    pass
    ...
    def run(self):
        ...
        while running:
            ...
            self.callback(self.jpeg.decode(block[0]), block[1],
                          block[2], block[3], block[4], block[5], self)  # 1.5x faster than ffmpeg embedded into opencv
...
def main():
    ...
    # Intercept signals
    signal.signal(signal.SIGINT, lambda signum, frame: exit_gracefully(sc, signum, frame))
    signal.signal(signal.SIGTERM, lambda signum, frame: exit_gracefully(sc, signum, frame))
