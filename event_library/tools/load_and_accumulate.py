...
from event_library.representations import BaseRepresentation
...
def accumulate_and_save(
    input_path: str, output_path: str, representation: BaseRepresentation
) -> np.array:
    events = utils.load_from_file(input_path, num_events=-1)
    os.makedirs(output_path, exist_ok=True)

    for i, frame in enumerate(representation.get_generator(events=events)):
        frame = normalized_3sigma(frame)
...
def main():
    ...
    dvs_info = el.utils.get_hw_property('dvs')  # DVS camera

    representation_args = {
        "num_events": args.num_events,
        "frame_size": (dvs_info.size[0], dvs_info.size[1]),
    }
    representation = representations.get_representation(args.representation)(
        **representation_args
    )
    ...
    for input_path in input_files:
        ...
        accumulate_and_save(input_path, out_path, representation)
